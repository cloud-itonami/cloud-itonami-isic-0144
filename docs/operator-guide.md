# Operator Guide: Sheep-and-Goat Farm Operations Coordinator

## Overview

The Sheep-and-Goat Farm Operations Coordinator is a facility-management robot that:

1. **Logs operational data** — flock counts, weights, health notes, offspring
   (lambing/kidding) and shearing (fleece) data
2. **Schedules coordination** — grazing-rotation, shearing, breeding operations,
   supply orders
3. **Escalates concerns** — any animal health or welfare issue (e.g. suspected scrapie)
...
1. **Facility Registration** — your paddock/pen facility must be registered in
...
3. **Clear Request Type** — specify what you're doing:
   - `:log-husbandry-record` — record flock data (feeding/breeding/shearing/health-check)
   - `:schedule-farm-operation` — arrange grazing-rotation/shearing/breeding operation
...
2. **Actor Processes** (`operation/run-operation store request context`)
   - `:advise` — `FlockOpsAdvisor` proposes an action (`flockops.advisor`)
   - `:govern` — `FlockOperationsGovernor` checks hard invariants and escalation gates (`flockops.governor`)
   - phase gate — rollout-phase constraints applied on top of the Governor's verdict (`flockops.phase`)
...
- `:flag-animal-health-concern` — any welfare or health issue (e.g. suspected scrapie)
...
- `:finalize-culling-decision` — economic/ethical decisions are human authority
- Missing/unregistered facility — must register first

### Resuming Escalated Operations

`flockops.operation` is currently a synchronous stub (see its docstring):
...
mirroring `cloud-itonami-isic-0145`). Until then, an `:escalate`
...
The actor provides a standard protocol (`flockops.store/Store`) for backend
integration:

- **Facility lookup** — `(store/registered-facility store facility-id)`

Implementations include in-memory `MemStore` (testing, `flockops.store`),
...
- **No unsupervised decisions** — no animal treatment or welfare decision is
  made by the robot
- **No suppressed concerns** — animal health/welfare concerns cannot be
  hidden or delayed
