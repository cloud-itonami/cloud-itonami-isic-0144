(ns flockops.advisor
  "FlockOpsAdvisor -- the contained LLM/decision node. This actor's
  intelligence layer proposes back-office coordination actions (flock
  husbandry record logging including feeding/breeding/shearing/health-
  check batch data, grazing-rotation/shearing/breeding scheduling,
  health/welfare-concern flags, supply procurement) based on facility
  state and operator input. The advisor is SEALED into the `:advise`
  step of the operation flow; every proposal is routed through the
      ...
      (case op
        ...
        {:op :log-husbandry-record
         ...
         :value {:facility-id facility-id
                 ...
                 :offspring-count (:offspring-count request nil)
                 :fleece-weight (:fleece-weight request nil)}
         :cites ["operator-submitted-count"]
         :summary "Flock feeding/breeding/shearing/health-check batch entry logged from operator submission"
         :confidence 0.9}
        ...
        {:op :schedule-farm-operation
         :effect :propose
         :value {:facility-id facility-id
                 :operation-type (:operation-type request "grazing-rotation")
                 :requested-date (:requested-date request)
                 :reason (:reason request "routine-operation")}
         :cites ["operator-scheduling-request"]
         :summary "Grazing-rotation/shearing/breeding operation proposed per operator request"
         :confidence 0.85}
        ...
        {:op :flag-animal-health-concern
         ...
         :summary "Animal health/welfare concern (e.g. suspected scrapie or bluetongue) flagged for veterinary/farm-operator review"
         :confidence 0.8}
        ...
        {:op :order-supplies
         ...
         :summary "Supply order (feed/veterinary-supply/shearing-equipment) proposed for facility"
         :confidence 0.85}
