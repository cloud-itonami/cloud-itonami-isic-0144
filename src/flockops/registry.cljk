(ns flockops.registry
  "Pure validation functions for sheep-and-goat farm operations. These are
...
(defn flock-count-non-positive?
  "A logged flock/head count of zero or negative is not a real
  observation -- reject it as a HARD violation rather than silently
  accepting bad data into the flock record."
  [count]
  (<= count 0))
