(ns flockops.operation
  "OperationActor -- one sheep-and-goat farm operation = one supervised
  actor run. The advisor (FlockOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Flock Operations
  ...
  One run = one sheep/goat farm coordination operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A facility's
  operating history is advanced by MANY operations (log-husbandry-record
  / schedule-farm-operation / flag-animal-health-concern /
  ...
  Governor is clean -- see `flockops.governor/always-escalate-ops`.
  ...
  (:require [flockops.advisor :as advisor]
...
(defn run-operation
  "Run one sheep-and-goat farm operation through the advisor -> governor
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- a `flockops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
