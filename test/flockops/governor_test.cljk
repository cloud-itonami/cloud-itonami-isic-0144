(ns flockops.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [flockops.governor :as gov]
...
(deftest hard-violations-effect-not-propose
  (testing "Hard violation: effect is not :propose"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
...
(deftest hard-violations-culling-blocked
  (testing "Hard violation: finalizing a culling decision is permanently blocked"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :finalize-culling-decision :effect :propose}
          verdict (gov/check req nil prop s)]
      (is (:hard? verdict))
      (is (some #(= :treatment-or-culling-blocked (:rule %)) (:violations verdict))))))
...
(deftest ok-husbandry-logging
  (testing "OK: valid husbandry record logging with a registered facility"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :log-husbandry-record :effect :propose :count 85 :confidence 0.9}
          verdict (gov/check req nil prop s)]
      (is (:ok? verdict))
      (is (not (:hard? verdict)))
...
(deftest escalation-health-concern
  (testing "Escalation: animal health/welfare concern (e.g. suspected scrapie) ALWAYS escalates, even at high confidence"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :flag-animal-health-concern :effect :propose
                :concern "スクレイピーの疑い" :confidence 0.95}
          verdict (gov/check req nil prop s)]
      (is (not (:hard? verdict)))
      (is (:escalate? verdict))
...
(deftest escalation-low-confidence
  (testing "Escalation: confidence below the floor"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :log-husbandry-record :effect :propose :count 85 :confidence 0.5}
          verdict (gov/check req nil prop s)]
      (is (not (:hard? verdict)))
      (is (:escalate? verdict)))))
...
(deftest escalation-supply-order-category-specific-threshold
  (testing "Escalation: supply order over its category-specific threshold (shearing-equipment: 800)"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :order-supplies :effect :propose :cost 900 :confidence 0.9
                :value {:category "shearing-equipment"}}
          verdict (gov/check req nil prop s)]
      (is (:escalate? verdict))))

  (testing "OK: shearing-equipment order under its higher category threshold"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
          prop {:op :order-supplies :effect :propose :cost 700 :confidence 0.9
                :value {:category "shearing-equipment"}}
          verdict (gov/check req nil prop s)]
      (is (:ok? verdict))
      (is (not (:escalate? verdict))))))
...
(deftest ok-schedule-farm-operation
  (testing "OK: scheduling a grazing-rotation/shearing/breeding operation is a routine coordination op"
    (let [facility {:id "farm-001" :name "Sunrise Flock Farm"}
          s (store/mem-store {:initial-facilities {"farm-001" facility}})
          req {:facility-id "farm-001"}
